...
$Z_j \sim N(0,1)$, $X_{jk} = Z_j + e_{1j}$, and $Y_j = Z_j + \sum_k X_{jk} \beta_k + e_{2j}$
where $e_1$ and $e_2$ are iid $N(0,1)$ say. 
So $$Y = Z_j (1+\sum_k \beta_k) + \sum_k \beta_k e_1 + e_2$$. 
...
First compute the covariance matrix of (Z,X1...,XK,Y).
```{r cov}
set.seed(1)
